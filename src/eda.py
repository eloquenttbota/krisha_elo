...
def eda_price(df: pd.DataFrame, colors: dict) -> None:
    ...
    card(
        ...
        note="Среднее выше медианы — цену тянет вверх небольшая группа очень дорогих "
             "квартир, вплоть до 1.6 млрд тг. Мы намеренно показываем весь диапазон "
             "без обрезки. «Выбросов по IQR» — это лишь грубая иллюстрация того, "
             "насколько скошено распределение, по стандартной статистической формуле; "
             "саму очистку данных мы сделаем иначе — реальными границами рынка Астаны "
             "(раздел 5) и перцентилями цены за м² (раздел 6), поэтому итоговые числа "
             "там будут другими. А для модели цену прологарифмируем, чтобы такие "
             "редкие дорогие квартиры не искажали обучение.",
        accent=colors["pos"],
    )
